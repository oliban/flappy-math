...
- `js/effects.js` - Escalating per-answer reward effects: streak tier ladder,
  sparks, stars, rings and firework shells
...
**Weather:** `server/weather.js` fetches current weather from Open-Meteo (no key). Location is Mölndal by default - the game never asks for the player's position and never looks it up by IP. The location card only opens when the player clicks the weather chip; accepting it sends browser coordinates (rounded to ~1 km) and "Use Mölndal" clears them again. Place names via BigDataCloud reverse geocoding. Cached in memory (10 min weather, 1 h geo). `js/weather.js` is the client; `js/background.js#setWeather` drives sky palette, cloud cover, rain/snow/fog/lightning, wind and night mode. `?weather=rain|snow|thunder|fog|overcast|clouds|clear` overrides for testing.
...
**Answer feedback:** every correct answer fires an effect tier chosen from the current streak (`js/effects.js`, 7 tiers: puff → sparkle → shockwave → jets → fountain → firework → finale), so a run of right answers visibly climbs and a mistake drops it back to tier 1. Effects render *behind* the pipes in `renderGame()` and their lifetime scales with the gap between pipes (`game.js#effectLifeScale`), so answers stay readable and nothing overlaps the next pipe. A wrong answer blinks the player's own avatar red (`bird.hurtFlash()`, `avatar-sprites.js#drawAvatarSpriteTint`) and snuffs the celebration (`effects.extinguish()`); pipe hits use the same blink, so there is one hurt look rather than two.
